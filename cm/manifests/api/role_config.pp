...
define cm::api::service_config($service_name, $role_name, $meta_data) {
  ...
  curl { "update-service-config-${prop_name}":
    ...
    put          => "/api/${api_version}/clusters/${cluster_name}/services/${service_name}/roles/${role_name}/config",
    parameters   => "{
                      'items' => [
                        {
                          'name' => '${prop_name}',
    ...
    returns      => '200',
    only_if      => {
                      'get' => "/api/${api_version}/clusters/${cluster_name}/services/${service_name}/roles/${role_name}/config",
                      'does_not_contain' => "$..name[?(@ == '${prop_name}')]"
                    },
    log_to      => "/var/log/ankus/cm_api/update_role_config_${role_name}"
  }
}
